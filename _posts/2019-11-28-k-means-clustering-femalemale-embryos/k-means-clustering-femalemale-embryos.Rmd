...
description: |
  In this post, I show how to distinguish female and male Drosophila embryos using k-means clustering.
author:
  - name: Tamas Schauer
date: 2019-11-20
output:
...
categories:
  ...
  - RNAseq
  - Drosophila
repository_url: https://github.com/tschauer/blog
---
...
```{r, layout = "l-body-outset", fig.height=10, fig.width=20}
# conditions
...
par(mfrow=c(1,4), oma=c(4,2,2,2), cex=1.5)
...
mtext(text = "Stages", side = 1, line = -2, outer = TRUE, cex = 2)
mtext(text = "log2 TPM", side = 2, outer = TRUE, cex = 2)

par(fig = c(0,1,0,1), mar=c(0,0,0,0), cex=1.5, new = TRUE)
